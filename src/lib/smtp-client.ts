...
export class SMTPClient {
  ...
  async verifyEmail(email: string, mxHost: string): Promise<SMTPVerificationResult> {
    try {
      ...
      // MAIL FROM command
      const mailFromResponse = await this.sendCommand(`MAIL FROM:<${this.fromEmail}>`)
      ...
      if (rcptToResponse.code >= 200 && rcptToResponse.code < 300) {
        ...
        return { success: false, response: rcptToResponse, error: "Temporary failure or greylisting" }
      }
    } catch (error) {
      this.disconnect()
      return {
